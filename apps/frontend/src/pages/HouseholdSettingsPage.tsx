...
export default function HouseholdSettingsPage() {
  ...
  const renameMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showSuccess('Household renamed successfully');
      setIsRenaming(false);
      setRenameValue('');
    },
  ...
  const createHouseholdMutation = useMutation({
    ...
    onSuccess: async (household) => {
      ...
      showSuccess(`'${household.name}' created and set as active`);
      setNewHouseholdName('');
  ...
  if (!activeHouseholdId) {
    return (
      <div className="p-6">
        <h1 className="text-3xl font-bold text-foreground mb-6">Household Settings</h1>
        <Card>
          <CardContent className="p-12 text-center">
            <p className="text-muted-foreground">You are not part of a household yet.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  if (isLoadingHouseholds || isLoadingDetails) {
    return (
    ...
    return (
      <div className="p-6">
        ...
        <div className="bg-destructive-subtle border border-destructive text-destructive-foreground px-4 py-3 rounded-md">
          Error loading household details: {(detailsError as Error).message}
        </div>
      </div>
    );
  }
