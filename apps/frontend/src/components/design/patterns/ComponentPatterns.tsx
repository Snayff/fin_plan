  ...
  return (
    <div className="space-y-12">
      ...
      <PatternSection
        ...
        useWhen={['The action is the primary CTA on the page (default)', 'You need a destructive action gate — always pair with ConfirmDialog']}
        avoidWhen={['Stacking more than one default (primary) button — one primary per section', 'Using destructive inline without confirming first']}
      >
        <PatternExample
          label="Variants"
...
<Button variant="secondary">Cancel</Button>`}
      ...
      <PatternSection
        ...
        useWhen={['Grouping a related set of data or controls', 'Any content that needs visual separation from the page background']}
        avoidWhen={['Nesting cards inside cards — flatten the hierarchy instead', 'Using a card for a single line of text']}
      >
        <PatternExample
...
{/* Note: use text-foreground for neutral balances. Reserve text-success (teal)
    for income figures or positive deltas, not plain account balances. */}`}
      ...
      <PatternSection
        ...
        useWhen={['Showing status (Active, Overdue, On track)', 'Labelling a category or type inline with other content']}
        avoidWhen={['Conveying urgency that needs a full Alert', 'Interactive actions — use Button instead']}
      >
        <PatternExample
          label="Variants"
...
<Badge className="bg-highlight text-highlight-foreground border-0">Goal</Badge>`}
  ...
  <p className="text-xs text-muted-foreground">Read-only — set by the system</p>
      ...
      <PatternSection
        ...
        useWhen={['Options are known and finite (5–15 items)', 'The field value must be one of the options']}
        avoidWhen={['Long lists (20+ items) — consider a searchable combobox', 'Binary yes/no — use a checkbox or toggle']}
      >
        <PatternExample
          label="Basic select"
          code={`<div className="space-y-1">
  ...
  <Select>
    ...
    <SelectContent>
      ...
      <PatternSection
        ...
        useWhen={['Showing a persistent warning or error on a page or form section', 'Informing the user about a state that persists until resolved']}
        avoidWhen={['Transient feedback after an action — use Toast', 'Success confirmation after a save — use Toast']}
      >
        <PatternExample
