...
const semanticColors = [
  ...
  },
  {
    ...
    hex: "#FF7A18",
    description: "Primary actions, CTAs, focus rings",
  },
  {
    name: "Success (Teal)",
    cssVar: "--success",
    ...
    hex: "#07BEB8",
    description: "Income, progress, positive states",
  },
  {
    name: "Accent (Purple)",
    cssVar: "--accent",
    ...
    hex: "#B38BA3",
    description: "Branding, accent features",
  },
  {
    name: "Attention (Amber)",
    cssVar: "--attention",
    tailwindClass: "bg-attention",
    hex: "#f59e0b",
    description: "Staleness, cashflow attention, nudge dots — the single attention signal",
  },
  {
    ...
    hex: "#E5484D",
    description: "App errors only — use sparingly",
  },
];
...
const typeSamples = [
  ...
  { label: "text-lg / 18px", className: "text-lg font-medium", text: "Large body / subheading" },
  {
    ...
    className: "text-base",
    text: "Body text — default. Mid-life users benefit from 16px base.",
  },
  {
    label: "text-sm / 14px",
    className: "text-sm text-muted-foreground",
    text: "Secondary info, labels, captions",
  },
  {
    label: "text-xs / 12px",
    className: "text-xs text-muted-foreground",
    text: "Fine print, badges, metadata",
  },
];
  ...
  return (
    <div className="space-y-12">
      ...
      <PatternSection
        ...
        useWhen={[
          "Communicating status or feedback (success, warning, error)",
          "Matching existing page patterns",
        ]}
        avoidWhen={[
          "Hardcoding hex values in className",
          "Using destructive for anything less than true errors or data loss",
        ]}
      >
        ...
        <div className="mt-6 p-4 rounded-lg bg-background border border-border text-sm space-y-2">
          ...
          <p className="text-muted-foreground text-xs">
            ...
            <code className="bg-card px-1 rounded">bg-attention-bg</code>. Use these for bordered
            info/alert blocks instead of full colour fills.
          </p>
        </div>
        <div className="mt-3 p-4 rounded-lg bg-background border border-border text-sm space-y-2">
          <p className="font-medium text-foreground">Attention (amber)</p>
          <p className="text-muted-foreground text-xs">
            Amber is the single attention signal:{" "}
            <code className="bg-card px-1 rounded">bg-attention</code>,{" "}
            <code className="bg-card px-1 rounded">bg-attention-bg</code>,{" "}
            <code className="bg-card px-1 rounded">border-attention-border</code>,{" "}
            <code className="bg-card px-1 rounded">text-attention</code>. Used for staleness,
            cashflow notes, and nudges.
          </p>
        </div>
      </PatternSection>
      ...
      >
        ...
        <div className="mt-4 p-4 rounded-lg bg-background border border-border text-sm">
          ...
          <p className="text-muted-foreground text-xs mb-2">
            JetBrains Mono is available for numerical data where alignment matters (e.g. financial
            figures).
          </p>
          <p className="font-mono text-base text-foreground">
            £12,450.00 · –£3,200.00 · +£8,900.50
          </p>
