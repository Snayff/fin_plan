    ...
    <div className="space-y-1.5 w-64">
      ...
      <div className="h-2 rounded-full bg-border overflow-hidden">
        <div
          ...
          style={{ width: `${pct}%`, backgroundColor: 'hsl(var(--income))' }}
        />
      </div>
      <div className="flex justify-between text-xs">
        <span className="font-mono text-income tabular-nums">£{used.toLocaleString()}</span>
        <span className="text-text-tertiary font-mono tabular-nums">£{remaining.toLocaleString()} remaining</span>
      </div>
    </div>
  );
}

const salaryHistory = [4500, 4500, 4800, 4800, 4800, 5000, 5000, 5148, 5148, 5148, 5148, 5148,
  5148, 5148, 5148, 5148, 5148, 5148, 5148, 5148, 5148, 5148, 5148, 5148];
const savingsHistory = [10000, 10700, 11400, 12100, 12800, 13500, 14200, 14900, 15600, 16300, 17000, 17300,
  17300, 17300, 17300, 17300, 17300, 17300, 17300, 17300, 17300, 17300, 17300, 17300];
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <PatternExample label="Savings balance — with snapshot marker (dashed line)">
            <div className="space-y-1">
              ...
              <p className="text-xs text-staleness">⚠ Last reviewed: 14 months ago</p>
              <DemoSparkline data={savingsHistory} color="hsl(var(--income))" snapshotIndex={11} />
              <div className="flex justify-between text-xs text-text-tertiary font-mono">
                <span>Mar 2024</span>
        ...
        <div className="space-y-4">
          ...
          <PatternExample label="Tier with no items — inline add action">
            <div className="space-y-2 text-sm">
              ...
              <button className="text-primary hover:underline text-sm text-left">+ Add first income source</button>
            </div>
          </PatternExample>
        </div>
      </PatternSection>
