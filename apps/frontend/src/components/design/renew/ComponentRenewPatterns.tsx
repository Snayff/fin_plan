...
function DemoCard({ children, stale = false }: { children: React.ReactNode; stale?: boolean }) {
  return (
    <div className={`rounded-md border bg-card p-6 ${stale ? 'border-staleness/40' : 'border-border'}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="space-y-12">
          ...
          <PatternExample label="Right panel detail view context — avatar + headline + metadata">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-3xl font-semibold font-mono text-income tabular-nums">£17,300</p>
        ...
        <div className="grid grid-cols-2 gap-4">
          <PatternExample label="Default card — surface bg + border">
            <DemoCard>
              ...
              <p className="text-2xl font-bold font-mono text-income tabular-nums">£5,148 / mo</p>
              <p className="text-xs text-text-tertiary mt-1">Last reviewed: Jan 2026</p>
              <div className="flex gap-2 mt-4">
                <button className="px-3 py-1.5 text-xs rounded border border-border text-foreground">Update</button>
                <button className="px-3 py-1.5 text-xs rounded bg-primary text-white">Still correct ✓</button>
              </div>
            </DemoCard>
          </PatternExample>
          <PatternExample label="Stale card — same bg, staleness amber applied to age indicator">
            <DemoCard stale>
              <div className="flex items-start justify-between">
                ...
                <span className="text-xs text-staleness font-mono">⚠ 14 months</span>
              </div>
              <p className="text-2xl font-bold font-mono text-income tabular-nums">£3,708 / mo</p>
              <p className="text-xs text-staleness mt-1">Last reviewed: Jan 2025</p>
              <div className="flex gap-2 mt-4">
                <button className="px-3 py-1.5 text-xs rounded border border-border text-foreground">Update</button>
                <button className="px-3 py-1.5 text-xs rounded bg-primary text-white">Still correct ✓</button>
              </div>
      ...
      <PatternSection
        ...
        avoidWhen={['Other pages', 'When in live/current view — SnapshotBanner and TimelineNavigator are mutually exclusive']}
      >
      ...
      <PatternSection
        ...
        description="Non-destructive error state when the app can't sync with the backend. Retains cached data. Uses staleness amber — never destructive red. Auto-dismisses on successful resync."
      ...
      >
        <PatternExample label="Muted amber bar — informational, never blocking">
          <div className="rounded-md px-4 py-2 flex items-center justify-between text-sm bg-staleness/10 border border-staleness/30">
            <span className="text-text-secondary">
              Data may be outdated — last synced{' '}
              <span className="text-staleness font-medium">3 minutes ago</span>
            </span>
