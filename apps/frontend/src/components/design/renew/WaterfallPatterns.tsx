...
function DemoWaterfallTierRow({
  ...
  return (
    <div
      className={`
        flex items-center justify-between px-3 py-2 rounded-sm cursor-pointer transition-colors
        ${selected ? 'bg-card border-l-2' : 'hover:bg-card/50 border-l-2 border-transparent'}
      `}
      style={selected ? { borderLeftColor: color } : undefined}
    >
      <div className="flex items-center gap-2">
        <span
          className={`font-semibold uppercase tracking-wide ${isSurplus ? 'text-base' : 'text-sm'}`}
          style={{ color }}
        >
          {name}
        </span>
        {staleBadge && (
          <span className="text-xs bg-staleness/20 text-staleness px-1.5 py-0.5 rounded-full">
            {staleBadge}
          </span>
        )}
        {shortfall && (
      ...
      <span
        className={`font-mono font-semibold tabular-nums ${isSurplus ? 'text-xl font-bold' : 'text-sm'}`}
  ...
  if (variant === 'dot') {
    return (
      <div className="flex items-center gap-2 text-sm">
        <span className="text-foreground">Josh Salary</span>
        <span className="w-1.5 h-1.5 rounded-full bg-staleness inline-block" title="Last reviewed: 14 months ago" />
        <span className="font-mono text-sm tabular-nums text-foreground">£5,148</span>
      </div>
    );
  }
  return (
    <div className="space-y-1">
      <p className="text-3xl font-semibold font-mono text-income tabular-nums">£5,148</p>
      <p className="text-xs text-text-tertiary">Last reviewed: 14 months ago</p>
    </div>
  );
}

function DemoButtonPair({
  left,
  right,
}: {
  left: string;
  ...
  return (
    <div className="flex gap-2">
      <button className="px-4 py-2 text-sm rounded-md border border-border text-foreground hover:bg-card transition-colors">
        {left}
      </button>
...
function DemoNudgeCard({ text, link }: { text: string; link?: string }) {
  return (
    <div className="p-3 rounded-md bg-card border border-border text-sm space-y-1">
...
export function WaterfallPatterns() {
  return (
    <div className="space-y-12">
      <div>
        <h2 className="text-2xl font-semibold text-foreground mb-2">Waterfall</h2>
        <p className="text-sm text-text-secondary">
          Components unique to the FinPlan waterfall mental model. The waterfall is the identity of the app — these
          components are the primary visual language.
        </p>
      </div>
      ...
      >
        <PatternExample>
          <div className="space-y-0 w-48 text-sm">
            <div className="px-3 py-1 font-semibold uppercase text-xs" style={{ color: 'hsl(var(--tier-income))' }}>
              INCOME &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; £8,856
            </div>
            <DemoWaterfallConnector label="minus committed spend" />
            <div className="px-3 py-1 font-semibold uppercase text-xs" style={{ color: 'hsl(var(--tier-committed))' }}>
              COMMITTED &nbsp;&nbsp;&nbsp; £4,817
            </div>
            <DemoWaterfallConnector label="minus discretionary" />
            <div className="px-3 py-1 font-semibold uppercase text-xs" style={{ color: 'hsl(var(--tier-discretionary))' }}>
              DISCRETIONARY £3,830
            </div>
            <div className="px-3 py-0.5 border-t border-border mt-0.5">
              <span className="text-xs text-text-tertiary font-mono">= equals</span>
            </div>
            <div className="px-3 py-1 font-bold text-base" style={{ color: 'hsl(var(--tier-surplus))' }}>
              SURPLUS &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; £209
            </div>
          </div>
        </PatternExample>
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          <PatternExample label="Row level — amber dot only, no text">
            <DemoStalenessIndicator variant="dot" />
            <p className="text-xs text-text-tertiary mt-2">Hover the dot to see: "Last reviewed: 14 months ago"</p>
          </PatternExample>
      ...
      <PatternSection
        ...
        useWhen={['Overview page only — above the left + right panels', 'When at least one snapshot exists']}
        avoidWhen={['Other pages', 'When viewing a snapshot — replaced by SnapshotBanner in that state']}
      >
