  ...
  it('throws on 409 conflict when email already exists', async () => {
    server.use(
      http.post('/api/auth/register', () =>
        HttpResponse.json({ error: { code: 'EMAIL_ALREADY_EXISTS', message: 'Email in use' } }, { status: 409 })
      )
    );
    await expect(
...
describe('authService.logout', () => {
  ...
  it('throws on 500 server error', async () => {
    ...
    await expect(authService.logout('mock-access-token')).rejects.toMatchObject({ statusCode: 500 });
  });
});
  ...
  it('throws on 401 when token is invalid', async () => {
    server.use(
      http.get('/api/auth/me', () =>
        HttpResponse.json({ error: { code: 'AUTHENTICATION_ERROR', message: 'Invalid token' } }, { status: 401 })
      )
    );
    await expect(authService.getCurrentUser('mock-access-token')).rejects.toMatchObject({ statusCode: 401 });
  });
});
  ...
  it('throws on 400 validation error', async () => {
    server.use(
      http.patch('/api/auth/me', () =>
        HttpResponse.json({ error: { code: 'VALIDATION_ERROR', message: 'Name is required' } }, { status: 400 })
      )
    );
    await expect(
  ...
  it('throws on 401 when refresh token is expired', async () => {
    server.use(
      http.post('/api/auth/refresh', () =>
        HttpResponse.json({ error: { code: 'AUTHENTICATION_ERROR', message: 'Refresh token expired' } }, { status: 401 })
      )
    );
