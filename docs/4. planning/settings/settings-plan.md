...
All 7 implemented sections are complete (Profile, Staleness, Surplus, ISA, Household, Snapshots, Waterfall Rebuild, Ended Income). Outstanding:

- [ ] TrustAccountsSection: add a settings section for managing trust account beneficiary names — the spec lists this as one of the 8 sections, but no component exists and the SettingsPage navigation does not include it
