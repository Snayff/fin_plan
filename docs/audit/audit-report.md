...
| 4   | Empty state shows CTA: "Set up your waterfall from scratch ▸"                                      | 🟡     | CTA shows "Build your waterfall" / "Get started" — copy differs from spec. **Triaged: accepted as intentional; current copy preferred.** |
...
**Triaged:** AC3 — Spec called for a floating card with "Current view" label + dot, but the [Now] button approach works well. **Button approach accepted; spec to be updated.**
...
| 8   | Right panel breadcrumb shows `← Tier / Item`                                  | ✅     | **Fixed.** Was showing tier only at item level. Now shows `← Tier / ItemName` (e.g. `← Income / Salary`). Group level already correct.                           |
...
| 4   | Bar row: `[◂]` button, scrollable track, `[▸]` button, `[Now]` button                | ✅     | **Triaged: false positive.** Buttons implemented but conditionally rendered when `canScrollLeft`/`canScrollRight` is true. Not visible with single snapshot.                       |
| 5   | `[◂]` and `[▸]` styled bordered buttons (28×28px, surface bg, radius border)         | ✅     | **Triaged: false positive.** Buttons present in code, hidden due to test data limitation.                                                                                          |
...
| 16  | "Read only" tag uses `attention` amber token with low-opacity background             | ✅     | **Fixed.** Was using non-existent `var(--color-attention)` inline style. Replaced with Tailwind `text-attention bg-attention-bg`.                                                  |
