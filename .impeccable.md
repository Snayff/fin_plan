...
- **Easing canon**: `ease-out-quart: cubic-bezier(0.25, 1, 0.5, 1)` for entrances/stagger/slides; `ease-out-quint: cubic-bezier(0.22, 1, 0.36, 1)` for pop-ins. Never bounce, elastic, or generic `ease`. Only animate `transform` + `opacity`.
- **Button press** (Pattern 3): base-component rule on all `<Button>` — `active:scale-[0.97]`, 150ms ease-out. For custom interactive surfaces, add `active:scale-[0.98]` manually when tactile feedback adds value.
- **Stagger entrance** (Pattern 1): use when 3+ sibling blocks have a natural reading order on mount (e.g. waterfall tiers). `staggerChildren: 0.06`, 250ms/block, `y:6→0 + opacity:0→1`, ease-out-quart. Framer Motion `variants` propagation. `initial={false}` when `usePrefersReducedMotion()`. Do NOT use for repeating list items or mid-session panel updates.
- **Delight glow** (Pattern 2): use sparingly for non-judgmental payoff moments on mount. Radial tier-colour overlay at 9% opacity, `opacity:0→1→0` over 2s, 500ms delay. Current instance: surplus > 0. Never use for financial value judgements or as an attention/warning signal.
